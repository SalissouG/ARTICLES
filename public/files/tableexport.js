...
;(function (window, undefined) {
    ...
    $.fn.tableExport = function (options) {
                    ...
                    csv: function (rdel, name) {
                        ...
                        var colD = '"' + $.fn.tableExport.csv.separator + '"',
                            dataURL = 'data:text/csv;charset=utf-8,' +
                                escape('"' + $rows.map(function (i, val) {
                                        var $cols = $(val).find('th, td');
                                        return $cols.map(function (i, val) {
		...
		document.getElementById('exportdownload').click();
	    }

	    function checkCaptionXLS(exportButton, theClass) {
                var $caption = $el.find('caption:not(.head)');
                $caption.length ? $caption.append(exportButton) : $el.prepend('<caption class="' + bootstrapSpacing + settings.position + '">' + exportButton + '</caption>');
		addListener(theClass);
		document.getElementById('exportdownload').click();
	    }	

            function createButton(dataURL, myFile, myContent, myClass) {
                var exportButton = "<a id='exportdownload' href='" + dataURL + "' download='" + myFile + "' role='button' class='" + bootstrapClass + bootstrapTheme + myClass + "'>" + myContent + "</a>";
                checkCaption(exportButton);
            }

            function createObjButton(dataObject, myContent, myClass) {
                var exportButton = "<a id='exportdownload' href='#' data-obj='" + dataObject + "' download='" + dataObject.name + "' role='button' class='" + bootstrapClass + bootstrapTheme + myClass + "'>" + myContent + "</a>";
		//checkCaption(exportButton);
		//addListener(myClass);
		checkCaptionXLS(exportButton, myClass);
            }
